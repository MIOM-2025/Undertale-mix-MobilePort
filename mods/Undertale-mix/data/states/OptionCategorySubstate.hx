...
var dragSensitivity:Float = 0.014; // 提高阻尼感（原0.025）
...
function create() {
    categories = data[1];
    if (data[2] != null && data[2].inputMode != null) {
        inputMode = data[2].inputMode;
    } else {
        #if mobile inputMode = "touch"; #else inputMode = "keyboard"; #end
    }
    ...
    for (category in categories) {
        ...
        button.ID = index;
        button.cameras = [data[0].mainCamera];
        add(button);
        ...
        switch(category.type) {
            case 'checkbox':
                ...
                optionType.cameras = [data[0].mainCamera];
                add(optionType);
            case 'slider':
    ...
    data[0].descriptionCamera.visible = true;
    updateSelection();
...
function update(elapsed:Float) {
    ...
    for (object in optionObjects) {
        ...
        text.setPosition(FlxMath.lerp(object.initX + object.add + (4 * (object.id == selected && !optionSelected ? 1 : -1)), text.x, lerp), 
    ...
    // ========== 拖拽逻辑 ==========
    if (!optionSelected && canChange && this.substate == null) {
        var mainCamera = data[0].mainCamera;
        var mousePointMain = FlxG.mouse.getWorldPosition(mainCamera);
        
        if (FlxG.mouse.justPressed && !isDragging) {
            for (object in optionObjects) {
                var text:UndertaleText = object.object;
                if (text.overlapsPoint(mousePointMain, false, mainCamera)) {
                    isDragging = true;
        ...
        if (isDragging && FlxG.mouse.pressed) {
            ...
            if (targetIndex != selected) {
                ...
                data[0].descriptionBox.resetAndChangeText(categories[selected].description, true);
                data[0].descriptionBox.startTyping(0.026, 'text-blip', true);
                data[0].descriptionBox.advanceDialogue();
        ...
        if (isDragging && FlxG.mouse.justReleased) {
            ...
            for (object in optionObjects) {
                var text:UndertaleText = object.object;
                if (text.visible && text.overlapsPoint(mousePointMain, false, mainCamera)) {
    ...
    if (categories[selected].title == 'Music Volume') {
        try { FlxG.sound.music.volume = optionObjects[selected].option.currentValue; } catch (e:Dynamic) { }
...
function performAccept() {
    ...
    if (categories[selected].title == 'Edit Song Offset') {
        if (canChange) openSubState(new ModSubState('OffsetEditorSubstate'));
    } else if (categories[selected].title == 'Edit Ratings') {
        if (canChange) openSubState(new ModSubState('RatingEditorSubstate'));
